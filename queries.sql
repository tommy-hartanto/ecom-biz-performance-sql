...
CREATE TABLE stage_2 (
	...
	customers_order_average DOUBLE PRECISION
	);


-- 1. Average of monthly active users (MUA):

INSERT INTO stage_2 (mau_avg, year) -- Insert results into table step_2
SELECT
...
FROM(
	...
	ORDER BY purchase_year_first) AS t2
	WHERE stage_2.year = t2.purchase_year_first;
...
FROM (
	...
	WHERE od.order_status = 'delivered' -- look for delivered orders only
) AS t1
...
FROM (
	...
	FROM (
		...
		FROM (
			...
			SUM(price + freight_value) AS revenue -- first, create new column revenue
			FROM orders_dataset AS od
			...
			GROUP BY purchase_year, product_category_name
		) AS t1
	) AS t2
...
SET year_2016 = payment_type_2016_sum
FROM (
	SELECT
	payment_type,
...
UPDATE stage_4 -- insert the values into the respective table
SET year_2017 = payment_type_2017_sum
FROM (
	SELECT
	payment_type,
...
UPDATE stage_4 -- insert the values into the respective table
SET year_2018 = payment_type_2018_sum
FROM (
	SELECT
	payment_type,
